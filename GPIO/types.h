/* ========================================================================== --
                     copyright, Polytech'Tours, 2019-2019
   ...
   History:
   --------------------------------------------------------------------------
   Date: 19.11.2014 - Author: Philippe Lamaty / Fabien Roger
   + First version
...
#define M_get64be(a)        ( (uint64_t)(((uint8_t *)a)[0]) << 56 \
                            | (uint64_t)(((uint8_t *)a)[1]) << 48 \
                            | (uint64_t)(((uint8_t *)a)[2]) << 40 \
                            | (uint64_t)(((uint8_t *)a)[3]) << 32 \
                            | (uint64_t)(((uint8_t *)a)[4]) << 24 \
                            | (uint64_t)(((uint8_t *)a)[5]) << 16 \
                            | (uint64_t)(((uint8_t *)a)[6]) <<  8 \
                            | (uint64_t)(((uint8_t *)a)[7])       \
                            )
...
#define M_field_reg(reg, msb, lsb, value)       (M_field_clear(reg, msb, lsb) | M_field_set(msb, lsb, value))
...
/**The bitfields can be defined with "define" keyword with a syntax a similar
   ...
   becomes:
        // coding of day on bits [31,27]
        #define FIELD_DATE_DAY       31,27
        // coding of month on bits [26,23]
        #define FIELD_DATE_MONTH     26,23
        // coding of year on bits [22,0]
...
#define M_bupdate(reg, field, value)    M_field_reg(reg, field, value)

/* Update to 'value' the field 'field' of 'reg' with cast on 1 byte */
#define M_bupdate8(reg, field, value)   (reg) = (uint8_t)M_field_reg(reg, field, value)

/* Update to 'value' the field 'field' of 'reg' with cast on 2 bytes */
#define M_bupdate16(reg, field, value)  (reg) = (uint16_t)M_field_reg(reg, field, value)

/* Update to 'value' the field 'field' of 'reg' with cast on 4 bytes */
#define M_bupdate32(reg, field, value)  (reg) = (uint32_t)M_field_reg(reg, field, value)

    /**------------------------------------------------------------------ */

#define M_infiniteLoop()                for( ; ; )
